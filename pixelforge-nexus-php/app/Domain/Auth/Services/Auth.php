...
class Auth implements Authenticatable
{
    ...
    {
        $twoFactorAuthentication = new TwoFactorAuth('PixelForge Nexus');
    ...
    /**
     * PixelForge Nexus RBAC Methods
     */

    /**
     * Check if current user has a specific permission
     */
    public static function userHasPermission(string $permissionName, ?int $projectId = null): bool
    {
        ...
        return $rbacService->userHasPermission($userId, $permissionName, $projectId);
    }

    /**
     * Check if current user has any of the specified permissions
     */
    public static function userHasAnyPermission(array $permissions, ?int $projectId = null): bool
    {
        $userId = session('userdata.id');
        if (!$userId) {
            return false;
        ...
        $rbacService = app()->make(RBACService::class);
        return $rbacService->userHasAnyPermission($userId, $permissions, $projectId);
    }

    /**
     * Check if current user has all specified permissions
     */
    public static function userHasAllPermissions(array $permissions, ?int $projectId = null): bool
    {
        $userId = session('userdata.id');
        if (!$userId) {
            return false;
        ...
        $rbacService = app()->make(RBACService::class);
        return $rbacService->userHasAllPermissions($userId, $permissions, $projectId);
    }

    /**
     * Get all permissions for current user
     */
    public static function getUserPermissions(?int $projectId = null): array
    {
        $userId = session('userdata.id');
        if (!$userId) {
            return [];
        ...
        $rbacService = app()->make(RBACService::class);
        return $rbacService->getUserPermissions($userId, $projectId);
    }
    ...
    {
        return self::userHasPermission('manage_system');
    }
    ...
    public static function canManageProject(int $projectId): bool
    {
        ...
        $rbacService = app()->make(RBACService::class);
        return $rbacService->canManageProject($userId, $projectId);
    }

    /**
     * Authorize user for permission or redirect to 403
     */
    public static function authOrRedirect(string|array $permissions, ?int $projectId = null): bool
    {
        $hasPermission = is_array($permissions)
            ? self::userHasAnyPermission($permissions, $projectId)
            : self::userHasPermission($permissions, $projectId);
